AltLda<-function(X, Y){	
    ...
    # Calculate the F-static mentioned in Dziuda, 2010.
    N = nrow(X)
    J = length(unique(Y))
    ...
    for(i in 1:NROW(scoreClass)){
    	scoreClass[i,]$class0Prob = (scoreClass[i,]$score - wbarj0) * (scoreClass[i,]$score - wbarj0) #- 2.45 * log(qj0)
		scoreClass[i,]$class1Prob = (scoreClass[i,]$score - wbarj1) * (scoreClass[i,]$score - wbarj1) #- 2.45 * log(qj1)
